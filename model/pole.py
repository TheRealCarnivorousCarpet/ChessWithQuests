...
class Pole:
    ...
    def __init__(self, souradnice : str ):
        if len(souradnice) != 2:
            raise ValueError(f"Nesprávné souřadnice '{souradnice}'")
    ...
    @classmethod
    def from_sloupec_rada( cls, s : int, r : int ) -> Self:
        if not 1 <= s <= 9 or not 1 <= r <= 9:
            raise ValueError(f"Coordinates out of bounds: ({s},{r})")
        return cls(f"{cls.SLOUPCE[s]}{cls.RADY[r]}")
